...
int us_libx264_encoder_compress(us_libx264_encoder_s *enc, const us_frame_s *src, us_frame_s *dest, bool force_key){
    ...
    switch (src->format) {
        case V4L2_PIX_FMT_YUYV:
        if (YUY2ToI420(src->data, src->width * 2,
            enc->picture_in->img.plane[0], src->width,
            enc->picture_in->img.plane[1], src->width / 2,
            enc->picture_in->img.plane[2], src->width / 2,
            src->width, src->height) != 0) {
                US_LOG_ERROR("H264 Encoder libx264: YUYV to I420 conversion failed!");
                return -1;
            }
            break;
        case V4L2_PIX_FMT_YUV420:
